...
def init_db():
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS service_requests (
        ...
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)""")
    # Seed admin
    if not conn.execute("SELECT id FROM agents WHERE email='admin@digitalmarketingagency.com'").fetchone():
        conn.execute("INSERT INTO agents (name,email,password_hash,role) VALUES (?,?,?,?)",
            ('Super Admin','admin@digitalmarketingagency.com', hashlib.sha256(b'Admin@123').hexdigest(),'admin'))
    conn.commit()
    conn.close()
